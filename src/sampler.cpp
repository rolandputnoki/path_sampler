...
void pathCallback(const Path p)
{
    ...
    transform.setOrigin( tf::Vector3(pose_array.poses[0].position.x,pose_array.poses[0].position.y , 0.0) );
    tf::Quaternion q;
    ...
    br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "world", "sampled"));
